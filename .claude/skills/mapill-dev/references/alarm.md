...
> Como o módulo funciona **hoje**, e não como se chegou até aqui. O caminho até cada decisão está
> no histórico do git e em `docs/HISTORICO-DE-PROGRESSO.md`.
>
> Escrito em 16/09, depois de os cenários do alarme passarem em aparelho. Movido de
> `docs/ARQUITETURA-DO-ALARME.md` para a skill em 25/09.
...
## Decisão fechada: a dose segue o instante, não a hora de parede

Decidido em 13/09. Um remédio cadastrado para as 21:00 em São Paulo toca às **20:00** em Manaus,
porque é o mesmo momento visto de outro fuso.

O motivo é clínico: **quem toma de 12 em 12 horas não deve encurtar o intervalo porque atravessou um
fuso.** Manter a hora de parede comprimiria ou esticaria o intervalo que a posologia estabelece.

Manter a hora de parede já foi implementado (regerar as doses ao detectar troca de fuso) e saiu. A
tabela `app_state` (migration 019) ficou, porque migration publicada não se remove.

**Se um dia for revisitado**, o caminho rigoroso é gravar a hora local pretendida (`"21:00"`) ao lado
do instante e derivar um do outro, e não detectar troca de fuso e regerar. Custa uma coluna,
backfill e a revisão dos arquivos que leem `scheduledFor`.
